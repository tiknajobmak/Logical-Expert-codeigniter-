jQuery(document).ready(function(){
    ...
    jQuery('#del_multiple').on('click',function(){
        deleteMultiple('classes/multipleDelete/');
        return false;
    });
    /** multiple delete end **/
    ...
    jQuery('#pagination_records').on('change',function(){
        ...
        var data = callAjax(ajaxData,'classes/ajaxCall/' , useBaseUrl = 1 );
        if(data != ''){
    ...
    jQuery('.ajaxData').on('click' , '.status' , function(){
        ...
        var data = callAjax(ajaxData , 'classes/changeStatus' , useBaseUrl = 1);
        if(data)
